...
4. `Esc` (with an empty search) stops all running tasks at once. To stop a single task, press its number / `↵` / `▶` (shown as `⏸` while running) again: only that task pauses, the others keep running. Logging out closes every open session.
